...
async function startServer() {
  ...
  // AI Route: Parse messy doctor dictation/text into structured clinical record & WhatsApp message
  app.post('/api/gemini/parse-note', async (req, res) => {
    try {
      ...
      if (!process.env.GEMINI_API_KEY) {
        // Fallback structuring if API key is not yet set
        return res.json({
...
4. WhatsApp Follow-up Message: A warm, polite, professional follow-up check-in message in respectful Urdu/English mix (e.g., beginning with "Assalam-o-Alaikum...") asking about their recovery and offering help.`;
      ...
      return res.json({
        ...
        whatsappMessage: parsed.whatsappMessage || `Assalam-o-Alaikum ${patientName || 'Patient'}, this is Al-Noor Clinic. Hope you are recovering well since your visit.`
      });
    } catch (error: any) {
